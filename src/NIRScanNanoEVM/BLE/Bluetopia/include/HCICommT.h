...
typedef struct _tagHCI_HCILLConfiguration_t
{
   ...
   /* the 3-Wire interface.  These parameters will only be applied during*/
   /* the link negotiation with the baseband that takes place when the  */
   /* first is sent or when the 3-wire state machines are reset via the */
   /* HCI_Reconfigure_Driver() function.                                */
...
typedef struct _tagHCI_SIBSConfiguration_t
{
   ...
   unsigned int         RetransmitTimeMs;       /* The time between message   */
                                                /* retransmissions.           */
   Boolean_t            AgressiveSleep;         /* This flag indicates        */
                                                /* that the SIBS instance     */
                                                /* should use Agressive Sleep */
