   ...
   if((BluetoothStackID) && (PatchLength) && (PatchData))
   {
      ...
      while((PatchLength) && (*PatchData == 0x01))
      {
         ...
         if((Result < 0) || (Status != 0))
         {
            ...
            DBG_DUMP(DBG_ZONE_VENDOR, (PatchData[3], &PatchData[4]));
   ...
   if((HCIDriverID) && (BluetoothStackID))
   {
      ...
      if(!VendorCommandsIssued)
      {
         ...
         if((BaudRate == BLUETOOTH_STARTUP_BAUD_RATE) || (!VS_Update_UART_Baud_Rate(BluetoothStackID, BaudRate)))
         {
            ...
            if((ret_val = Download_Patch(BluetoothStackID, BasePatchLength, BasePatch)) == TRUE)
            {
               ...
               ret_val = Download_Patch(BluetoothStackID, LowEnergyPatchLength, LowEnergyPatch);
            ...
            /* If the vendor specific initialization was successful.    */
            if((ret_val) && ((COMMProtocol == cpHCILL) || (COMMProtocol == cpHCILL_RTS_CTS)))
            {
               /* Configure HCILL since the user specified the protocol.*/
               ret_val = ConfigureHCILL(BluetoothStackID);
            }
         }
         else
         {
            DBG_MSG(DBG_ZONE_VENDOR, ("Update Baud Rate Failed.\r\n"));
         ...
         if(ret_val)
         {
            ...
            /* Flag that we have issued the Vendor Specific Commands (so*/
            /* we don't do it again if someone issues an HCI_Reset().   */
