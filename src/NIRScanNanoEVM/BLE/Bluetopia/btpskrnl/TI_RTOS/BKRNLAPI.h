...
typedef void *Mailbox_t;
   ...
   /* The following type declaration represents the retrievable state   */
   /* information for a Memory Pool.  This structure is used with the   */
   /* BTPS_QueryMemoryUsage() function contained in this module.        */
typedef struct _tagMemoryPoolUsage_t
{
   unsigned int BufferSize;
   unsigned int TotalNumberBuffers;
   unsigned int NumberFreeBuffers;
   unsigned int MaximumNumberBuffersUsed;
} MemoryPoolUsage_t;
...
typedef void (BTPSAPI *BTPS_MailboxDeleteCallback_t)(void *MailboxData);
   ...
   /* The following defines the callback function that is used to       */
   /* dispatch debug prints to the upper layer.                         */
typedef void (*DebugCallback_t)(unsigned char *Msg, int Length);
...
BTPSAPI_DECLARATION void BTPSAPI BTPS_CheckHeap(void);
#endif
